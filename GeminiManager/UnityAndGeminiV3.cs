...
{
    bool afficherBtn=true;

    [Header("Partie pour L'affichage")]
    [SerializeField] GameObject btnAfficher;
    [SerializeField] GameObject zoneTexteAfficher;
    ...
    [Header("ChatBot Fonction")]
    public TMP_InputField inputField;
    public TMP_Text uiText;
    private TextContent[] chatHistory;

    [Header("Prompt Fonction")]
    public string prompt = "";

    [Header("Image Prompt Fonction")]
    ...
    public void SendChat()
    {
        ...
        if (CalculerPtQuiz.objetClick != null)
        {
            afficherBtn = false;
            btnAfficher.SetActive(false);
            zoneTexteAfficher.SetActive(true);
            string userMessage = "Quel es le role du " + CameraGroupe.nomObjetMoteur + " dans le frigorifique?";
...
private IEnumerator SendPromptRequestToGeminiImageGenerator(string promptText)
{
            ...
            try 
            {
                    ...
                    response.candidates[0].content.parts != null)
                {
                    foreach (var part in response.candidates[0].content.parts)
                    {
                        ...
                        {
                            Debug.Log("Texte response: " + part.text);
                        }
    ...
    public void QuitterChatBot()
    {
        zoneTexteAfficher.SetActive(false);
        afficherBtn=true;
        if (CalculerPtQuiz.objetClick != null) {
            btnAfficher.SetActive(true);
        }
        else
        {
            btnAfficher.SetActive(false);
    ...
    private void Update()
    {
        //Debug.Log(CalculerPtQuiz.objetClick != null && afficherBtn);
        if (CalculerPtQuiz.objetClick != null && afficherBtn)
        {
            btnAfficher.SetActive(true);
        }
        else {
